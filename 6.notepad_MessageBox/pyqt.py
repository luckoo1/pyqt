...
class WindowClass(QMainWindow, form_class):
    ...
    def save_changed_data(self):
        ...
        msgBox.addButton('저장',QMessageBox.YesRole)      # QMessageBox.YesRole : 0
        msgBox.addButton('저장 안함',QMessageBox.NoRole)  # QMessageBox.NoRole1 : 1
        msgBox.addButton('취소',QMessageBox.RejectRole)   # QMessageBox.RejectRole : 2
        ret = msgBox.exec_() #실행
        print(ret)
        return ret
    ...
    def closeEvent(self,event):
        ...
        if ret == 2:  #취소버튼 누르면 event가 무시됨 -> 안꺼짐
            event.ignore()
        print("close")
